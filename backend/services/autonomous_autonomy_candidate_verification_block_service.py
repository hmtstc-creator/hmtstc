...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision in {301, 305} and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision in {302, 305} and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {303, 305} and not state['owner_approval']:
    ...
    if revision in {304, 305} and not state['halt_authority']:
    ...
    if revision in {305} and (state['strategy_degraded'] or state['underperformance_count'] >= 3):
    ...
    body = {
        'decision': decision,
        'autonomy_candidate_verification': decision,
        'revision': revision,
        'label': label,
    ...
    return {'engine': 'autonomy_candidate_verification', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'rev305_packet_or_next_block'}

def build_rev301_candidate_evidence_recheck(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(301, 'candidate_evidence_recheck', 'Rev300 candidate evidence recheck', data, settings, auth_store, username)

def build_rev302_permission_ladder_consistency_validator(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(302, 'permission_ladder_consistency_validator', 'Autonomy permission ladder consistency validator', data, settings, auth_store, username)

def build_rev303_halt_freeze_reduce_authority_proof(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(303, 'halt_freeze_reduce_authority_proof', 'Halt/freeze/reduce authority proof', data, settings, auth_store, username)

def build_rev304_summary_compact_decision_consistency_check(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(304, 'summary_compact_decision_consistency_check', 'Summary compact decision consistency check', data, settings, auth_store, username)

def build_rev305_autonomy_candidate_verification_packet(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(305, 'autonomy_candidate_verification_packet', 'Autonomy candidate verification packet', data, settings, auth_store, username)


_BUILDERS = {
    301: build_rev301_candidate_evidence_recheck,
    302: build_rev302_permission_ladder_consistency_validator,
    303: build_rev303_halt_freeze_reduce_authority_proof,
    304: build_rev304_summary_compact_decision_consistency_check,
    305: build_rev305_autonomy_candidate_verification_packet,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for Autonomy Candidate Verification') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'autonomy_candidate_verification': body.get('autonomy_candidate_verification') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(301, 305+1)}
    final = parts[305].get(_REV_KEYS[305], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'autonomy_candidate_verification',
        'revision': 305,
        ...
        'rev301_candidate_evidence_recheck': parts[301],
        'rev302_permission_ladder_consistency_validator': parts[302],
        'rev303_halt_freeze_reduce_authority_proof': parts[303],
        'rev304_summary_compact_decision_consistency_check': parts[304],
        'rev305_autonomy_candidate_verification_packet': parts[305],
        'autonomy_candidate_verification_packet': final,
        'summary_result': build_summary_for_revision(305, data, settings, auth_store, username),
        'checks': checks,
