...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision in {321, 325} and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision in {322, 325} and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {323, 325} and not state['owner_approval']:
    ...
    if revision in {324, 325} and not state['halt_authority']:
    ...
    if revision in {321, 322, 323, 324, 325} and (state['strategy_degraded'] or state['underperformance_count'] >= 3):
    ...
    return {'engine': 'strategy_survival_kill_switch', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'rev325_packet_or_next_block'}

def build_rev321_strategy_survival_score(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(321, 'strategy_survival_score', 'Strategy survival score', data, settings, auth_store, username)

def build_rev322_consecutive_underperformance_detector(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(322, 'consecutive_underperformance_detector', 'Consecutive underperformance detector', data, settings, auth_store, username)

def build_rev323_strategy_symbol_kill_switch_preview(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(323, 'strategy_symbol_kill_switch_preview', 'Strategy-symbol kill-switch preview', data, settings, auth_store, username)

def build_rev324_quarantine_downgrade_decision_controller(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(324, 'quarantine_downgrade_decision_controller', 'Quarantine / downgrade decision controller', data, settings, auth_store, username)

def build_rev325_strategy_survival_decision_packet(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(325, 'strategy_survival_decision_packet', 'Strategy survival decision packet', data, settings, auth_store, username)


_BUILDERS = {
    321: build_rev321_strategy_survival_score,
    322: build_rev322_consecutive_underperformance_detector,
    323: build_rev323_strategy_symbol_kill_switch_preview,
    324: build_rev324_quarantine_downgrade_decision_controller,
    325: build_rev325_strategy_survival_decision_packet,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for Strategy Survival & Kill-Switch') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'strategy_survival': body.get('strategy_survival') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(321, 325+1)}
    final = parts[325].get(_REV_KEYS[325], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'strategy_survival_kill_switch',
        'revision': 325,
        ...
        'rev321_strategy_survival_score': parts[321],
        'rev322_consecutive_underperformance_detector': parts[322],
        'rev323_strategy_symbol_kill_switch_preview': parts[323],
        'rev324_quarantine_downgrade_decision_controller': parts[324],
        'rev325_strategy_survival_decision_packet': parts[325],
        'strategy_survival_decision_packet': final,
        'summary_result': build_summary_for_revision(325, data, settings, auth_store, username),
        'checks': checks,
