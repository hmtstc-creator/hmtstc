...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision >= 361 and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision >= 365 and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {361, 362, 363, 364, 365} and not state['owner_approval']:
    ...
    if revision in {365} and not state['session_bound']:
    ...
    if revision in {361, 362, 363, 364, 365} and (not state['permission_ok'] or not state['whitelist_ok']):
    ...
    if revision in {365} and not state['halt_authority']:
    ...
    body = {
        'decision': decision,
        'controlled_growth_permission': decision,
        'revision': revision,
        'label': label,
    ...
    return {'engine': 'controlled_growth_permission', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'next_controlled_block'}

def build_rev361_repeat_permission_evidence_threshold(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(361, 'repeat_permission_evidence_threshold', 'Repeat Permission Evidence Threshold', data, settings, auth_store, username)

def build_rev362_minimum_sample_size_growth_blocker(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(362, 'minimum_sample_size_growth_blocker', 'Minimum Sample Size Growth Blocker', data, settings, auth_store, username)

def build_rev363_controlled_notional_increase_preview(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(363, 'controlled_notional_increase_preview', 'Controlled Notional Increase Preview', data, settings, auth_store, username)

def build_rev364_growth_blocked_by_default_proof(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(364, 'growth_blocked_by_default_proof', 'Growth Blocked By Default Proof', data, settings, auth_store, username)

def build_rev365_controlled_growth_permission_packet(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(365, 'controlled_growth_permission_packet', 'Controlled Growth Permission Packet', data, settings, auth_store, username)


_BUILDERS = {
    361: build_rev361_repeat_permission_evidence_threshold,
    362: build_rev362_minimum_sample_size_growth_blocker,
    363: build_rev363_controlled_notional_increase_preview,
    364: build_rev364_growth_blocked_by_default_proof,
    365: build_rev365_controlled_growth_permission_packet,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for Controlled Growth Permission') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'controlled_growth_permission': body.get('controlled_growth_permission') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(361, 365+1)}
    final = parts[365].get(_REV_KEYS[365], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'controlled_growth_permission',
        'revision': 365,
        ...
        'rev361_repeat_permission_evidence_threshold': parts[361],
        'rev362_minimum_sample_size_growth_blocker': parts[362],
        'rev363_controlled_notional_increase_preview': parts[363],
        'rev364_growth_blocked_by_default_proof': parts[364],
        'rev365_controlled_growth_permission_packet': parts[365],
        'controlled_growth_permission_packet': final,
        'summary_result': build_summary_for_revision(365, data, settings, auth_store, username),
        'checks': checks,
