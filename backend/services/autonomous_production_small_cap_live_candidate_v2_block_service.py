...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision >= 366 and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision >= 366 and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {366, 367, 368, 369, 370} and not state['owner_approval']:
    ...
    if revision in {366, 367, 368, 369, 370} and not state['session_bound']:
    ...
    if revision in {366, 367, 368, 369, 370} and (not state['permission_ok'] or not state['whitelist_ok']):
    ...
    if revision in {370} and not state['halt_authority']:
    ...
    body = {
        'decision': decision,
        'production_small_cap_live_candidate_v2': decision,
        'revision': revision,
        'label': label,
    ...
    return {'engine': 'production_small_cap_live_candidate_v2', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'next_controlled_block'}

def build_rev366_production_small_cap_live_checklist_v2(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(366, 'production_small_cap_live_checklist_v2', 'Production Small Cap Live Checklist V2', data, settings, auth_store, username)

def build_rev367_live_risk_capital_permission_combined_final_gate(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(367, 'live_risk_capital_permission_combined_final_gate', 'Live Risk Capital Permission Combined Final Gate', data, settings, auth_store, username)

def build_rev368_deploy_safe_runtime_package_audit_v2(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(368, 'deploy_safe_runtime_package_audit_v2', 'Deploy Safe Runtime Package Audit V2', data, settings, auth_store, username)

def build_rev369_rollback_halt_freeze_final_readiness(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(369, 'rollback_halt_freeze_final_readiness', 'Rollback Halt Freeze Final Readiness', data, settings, auth_store, username)

def build_rev370_production_small_cap_live_candidate_v2_packet(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(370, 'production_small_cap_live_candidate_v2_packet', 'Production Small Cap Live Candidate V2 Packet', data, settings, auth_store, username)


_BUILDERS = {
    366: build_rev366_production_small_cap_live_checklist_v2,
    367: build_rev367_live_risk_capital_permission_combined_final_gate,
    368: build_rev368_deploy_safe_runtime_package_audit_v2,
    369: build_rev369_rollback_halt_freeze_final_readiness,
    370: build_rev370_production_small_cap_live_candidate_v2_packet,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for Production Small-Cap Live Candidate v2') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'production_small_cap_live_candidate_v2': body.get('production_small_cap_live_candidate_v2') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(366, 370+1)}
    final = parts[370].get(_REV_KEYS[370], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'production_small_cap_live_candidate_v2',
        'revision': 370,
        ...
        'rev366_production_small_cap_live_checklist_v2': parts[366],
        'rev367_live_risk_capital_permission_combined_final_gate': parts[367],
        'rev368_deploy_safe_runtime_package_audit_v2': parts[368],
        'rev369_rollback_halt_freeze_final_readiness': parts[369],
        'rev370_production_small_cap_live_candidate_v2_packet': parts[370],
        'production_small_cap_live_candidate_v2_packet': final,
        'summary_result': build_summary_for_revision(370, data, settings, auth_store, username),
        'checks': checks,
