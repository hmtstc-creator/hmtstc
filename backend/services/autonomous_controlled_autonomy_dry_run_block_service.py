...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision in {306, 310} and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision in {307, 310} and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {308, 310} and not state['owner_approval']:
    ...
    if revision in {309, 310} and not state['halt_authority']:
    ...
    if revision in {310} and (state['strategy_degraded'] or state['underperformance_count'] >= 3):
    ...
    body = {
        'decision': decision,
        'controlled_autonomy_dry_run': decision,
        'revision': revision,
        'label': label,
    ...
    return {'engine': 'controlled_autonomy_dry_run', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'rev310_packet_or_next_block'}

def build_rev306_small_cap_autonomy_dry_run_session_planner(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(306, 'small_cap_autonomy_dry_run_session_planner', 'Small-cap autonomy dry-run session planner', data, settings, auth_store, username)

def build_rev307_dry_run_opportunity_to_intent_loop(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(307, 'dry_run_opportunity_to_intent_loop', 'Dry-run opportunity-to-intent loop', data, settings, auth_store, username)

def build_rev308_dry_run_risk_approval_capital_gate(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(308, 'dry_run_risk_approval_capital_gate', 'Dry-run risk approval and capital gate', data, settings, auth_store, username)

def build_rev309_dry_run_halt_freeze_reduce_simulation(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(309, 'dry_run_halt_freeze_reduce_simulation', 'Dry-run halt/freeze/reduce simulation', data, settings, auth_store, username)

def build_rev310_controlled_autonomy_dry_run_report(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(310, 'controlled_autonomy_dry_run_report', 'Controlled autonomy dry-run report', data, settings, auth_store, username)


_BUILDERS = {
    306: build_rev306_small_cap_autonomy_dry_run_session_planner,
    307: build_rev307_dry_run_opportunity_to_intent_loop,
    308: build_rev308_dry_run_risk_approval_capital_gate,
    309: build_rev309_dry_run_halt_freeze_reduce_simulation,
    310: build_rev310_controlled_autonomy_dry_run_report,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for Controlled Autonomy Dry-Run') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'controlled_autonomy_dry_run': body.get('controlled_autonomy_dry_run') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(306, 310+1)}
    final = parts[310].get(_REV_KEYS[310], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'controlled_autonomy_dry_run',
        'revision': 310,
        ...
        'rev306_small_cap_autonomy_dry_run_session_planner': parts[306],
        'rev307_dry_run_opportunity_to_intent_loop': parts[307],
        'rev308_dry_run_risk_approval_capital_gate': parts[308],
        'rev309_dry_run_halt_freeze_reduce_simulation': parts[309],
        'rev310_controlled_autonomy_dry_run_report': parts[310],
        'controlled_autonomy_dry_run_report': final,
        'summary_result': build_summary_for_revision(310, data, settings, auth_store, username),
        'checks': checks,
