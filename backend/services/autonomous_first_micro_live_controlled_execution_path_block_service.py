...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision >= 350 and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision >= 350 and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {346, 347, 348, 349, 350} and not state['owner_approval']:
    ...
    if revision in {346, 347, 348, 349, 350} and not state['session_bound']:
    ...
    if revision in {350} and not state['exit_plan_ready']:
    ...
    if revision in {350} and (not state['permission_ok'] or not state['whitelist_ok']):
    ...
    if revision in {350} and not state['halt_authority']:
    ...
    body = {
        'decision': decision,
        'first_micro_live_execution_path': decision,
        'revision': revision,
        'label': label,
    ...
    return {'engine': 'first_micro_live_controlled_execution_path', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'next_controlled_block'}

def build_rev346_first_micro_live_execution_intent_finalizer(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(346, 'first_micro_live_execution_intent_finalizer', 'First Micro Live Execution Intent Finalizer', data, settings, auth_store, username)

def build_rev347_submit_path_explicit_enable_proof(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(347, 'submit_path_explicit_enable_proof', 'Submit Path Explicit Enable Proof', data, settings, auth_store, username)

def build_rev348_order_preview_to_execution_contract(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(348, 'order_preview_to_execution_contract', 'Order Preview To Execution Contract', data, settings, auth_store, username)

def build_rev349_execution_blocked_by_default_proof(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(349, 'execution_blocked_by_default_proof', 'Execution Blocked By Default Proof', data, settings, auth_store, username)

def build_rev350_first_micro_live_execution_path_report(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(350, 'first_micro_live_execution_path_report', 'First Micro Live Execution Path Report', data, settings, auth_store, username)


_BUILDERS = {
    346: build_rev346_first_micro_live_execution_intent_finalizer,
    347: build_rev347_submit_path_explicit_enable_proof,
    348: build_rev348_order_preview_to_execution_contract,
    349: build_rev349_execution_blocked_by_default_proof,
    350: build_rev350_first_micro_live_execution_path_report,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for First Micro-Live Controlled Execution Path') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'first_micro_live_execution_path': body.get('first_micro_live_execution_path') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(346, 350+1)}
    final = parts[350].get(_REV_KEYS[350], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'first_micro_live_controlled_execution_path',
        'revision': 350,
        ...
        'rev346_first_micro_live_execution_intent_finalizer': parts[346],
        'rev347_submit_path_explicit_enable_proof': parts[347],
        'rev348_order_preview_to_execution_contract': parts[348],
        'rev349_execution_blocked_by_default_proof': parts[349],
        'rev350_first_micro_live_execution_path_report': parts[350],
        'first_micro_live_execution_path_report': final,
        'summary_result': build_summary_for_revision(350, data, settings, auth_store, username),
        'checks': checks,
