...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision >= 345 and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision >= 345 and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {341, 342, 343, 344, 345} and not state['owner_approval']:
    ...
    if revision in {341, 342, 343, 344, 345} and not state['session_bound']:
    ...
    if revision in {341, 342, 343, 344, 345} and state['command_expired']:
    ...
    if revision in {341, 342, 343, 344, 345} and (not state['permission_ok'] or not state['whitelist_ok']):
    ...
    if revision in {345} and not state['halt_authority']:
    ...
    body = {
        'decision': decision,
        'live_activation_command_decision': decision,
        'revision': revision,
        'label': label,
    ...
    return {'engine': 'live_activation_command_contract', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'next_controlled_block'}

def build_rev341_live_activation_command_schema(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(341, 'live_activation_command_schema', 'Live Activation Command Schema', data, settings, auth_store, username)

def build_rev342_owner_approval_command_scope_binder(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(342, 'owner_approval_command_scope_binder', 'Owner Approval Command Scope Binder', data, settings, auth_store, username)

def build_rev343_activation_expiry_stale_command_guard(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(343, 'activation_expiry_stale_command_guard', 'Activation Expiry Stale Command Guard', data, settings, auth_store, username)

def build_rev344_command_to_session_audit_contract(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(344, 'command_to_session_audit_contract', 'Command To Session Audit Contract', data, settings, auth_store, username)

def build_rev345_live_activation_command_decision_packet(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(345, 'live_activation_command_decision_packet', 'Live Activation Command Decision Packet', data, settings, auth_store, username)


_BUILDERS = {
    341: build_rev341_live_activation_command_schema,
    342: build_rev342_owner_approval_command_scope_binder,
    343: build_rev343_activation_expiry_stale_command_guard,
    344: build_rev344_command_to_session_audit_contract,
    345: build_rev345_live_activation_command_decision_packet,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for Live Activation Command Contract') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'live_activation_command_decision': body.get('live_activation_command_decision') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(341, 345+1)}
    final = parts[345].get(_REV_KEYS[345], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'live_activation_command_contract',
        'revision': 345,
        ...
        'rev341_live_activation_command_schema': parts[341],
        'rev342_owner_approval_command_scope_binder': parts[342],
        'rev343_activation_expiry_stale_command_guard': parts[343],
        'rev344_command_to_session_audit_contract': parts[344],
        'rev345_live_activation_command_decision_packet': parts[345],
        'live_activation_command_decision_packet': final,
        'summary_result': build_summary_for_revision(345, data, settings, auth_store, username),
        'checks': checks,
