...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision in {331, 335} and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision in {332, 335} and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {333, 335} and not state['owner_approval']:
    ...
    if revision in {334, 335} and not state['halt_authority']:
    ...
    if revision in {335} and (state['strategy_degraded'] or state['underperformance_count'] >= 3):
    ...
    body = {
        'decision': decision,
        'small_cap_limited_autonomy_candidate': decision,
        'revision': revision,
        'label': label,
    ...
    return {'engine': 'first_small_cap_limited_autonomy_candidate', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'rev335_packet_or_next_block'}

def build_rev331_small_cap_autonomy_final_checklist(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(331, 'small_cap_autonomy_final_checklist', 'Small-cap autonomy final checklist', data, settings, auth_store, username)

def build_rev332_first_limited_autonomy_session_contract(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(332, 'first_limited_autonomy_session_contract', 'First limited autonomy session contract', data, settings, auth_store, username)

def build_rev333_autonomy_rollback_freeze_protocol_finalizer(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(333, 'autonomy_rollback_freeze_protocol_finalizer', 'Autonomy rollback/freeze protocol finalizer', data, settings, auth_store, username)

def build_rev334_deploy_safe_autonomy_package_audit(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(334, 'deploy_safe_autonomy_package_audit', 'Deploy-safe autonomy package audit', data, settings, auth_store, username)

def build_rev335_first_small_cap_limited_autonomy_candidate_packet(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(335, 'first_small_cap_limited_autonomy_candidate_packet', 'First small-cap limited autonomy candidate packet', data, settings, auth_store, username)


_BUILDERS = {
    331: build_rev331_small_cap_autonomy_final_checklist,
    332: build_rev332_first_limited_autonomy_session_contract,
    333: build_rev333_autonomy_rollback_freeze_protocol_finalizer,
    334: build_rev334_deploy_safe_autonomy_package_audit,
    335: build_rev335_first_small_cap_limited_autonomy_candidate_packet,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for First Small-Cap Limited Autonomy Candidate') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'small_cap_limited_autonomy_candidate': body.get('small_cap_limited_autonomy_candidate') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(331, 335+1)}
    final = parts[335].get(_REV_KEYS[335], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'first_small_cap_limited_autonomy_candidate',
        'revision': 335,
        ...
        'rev331_small_cap_autonomy_final_checklist': parts[331],
        'rev332_first_limited_autonomy_session_contract': parts[332],
        'rev333_autonomy_rollback_freeze_protocol_finalizer': parts[333],
        'rev334_deploy_safe_autonomy_package_audit': parts[334],
        'rev335_first_small_cap_limited_autonomy_candidate_packet': parts[335],
        'first_small_cap_limited_autonomy_candidate_packet': final,
        'summary_result': build_summary_for_revision(335, data, settings, auth_store, username),
        'checks': checks,
