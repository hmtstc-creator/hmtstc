...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision in {316, 320} and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision in {317, 320} and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {318, 320} and not state['owner_approval']:
    ...
    if revision in {319, 320} and not state['halt_authority']:
    ...
    if revision in {320} and (state['strategy_degraded'] or state['underperformance_count'] >= 3):
    ...
    body = {
        'decision': decision,
        'capital_defense_runtime': decision,
        'revision': revision,
        'label': label,
    ...
    return {'engine': 'capital_defense_runtime', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'rev320_packet_or_next_block'}

def build_rev316_usdt_reserve_runtime_monitor(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(316, 'usdt_reserve_runtime_monitor', 'USDT reserve runtime monitor', data, settings, auth_store, username)

def build_rev317_profit_lock_runtime_enforcement_preview(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(317, 'profit_lock_runtime_enforcement_preview', 'Profit lock runtime enforcement preview', data, settings, auth_store, username)

def build_rev318_drawdown_shrink_runtime_controller(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(318, 'drawdown_shrink_runtime_controller', 'Drawdown shrink runtime controller', data, settings, auth_store, username)

def build_rev319_exposure_drift_detector(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(319, 'exposure_drift_detector', 'Exposure drift detector', data, settings, auth_store, username)

def build_rev320_capital_defense_runtime_packet(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(320, 'capital_defense_runtime_packet', 'Capital defense runtime packet', data, settings, auth_store, username)


_BUILDERS = {
    316: build_rev316_usdt_reserve_runtime_monitor,
    317: build_rev317_profit_lock_runtime_enforcement_preview,
    318: build_rev318_drawdown_shrink_runtime_controller,
    319: build_rev319_exposure_drift_detector,
    320: build_rev320_capital_defense_runtime_packet,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for Autonomous Capital Defense Runtime') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'capital_defense_runtime': body.get('capital_defense_runtime') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(316, 320+1)}
    final = parts[320].get(_REV_KEYS[320], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'capital_defense_runtime',
        'revision': 320,
        ...
        'rev316_usdt_reserve_runtime_monitor': parts[316],
        'rev317_profit_lock_runtime_enforcement_preview': parts[317],
        'rev318_drawdown_shrink_runtime_controller': parts[318],
        'rev319_exposure_drift_detector': parts[319],
        'rev320_capital_defense_runtime_packet': parts[320],
        'capital_defense_runtime_packet': final,
        'summary_result': build_summary_for_revision(320, data, settings, auth_store, username),
        'checks': checks,
