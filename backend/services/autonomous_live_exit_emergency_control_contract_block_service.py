...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision >= 355 and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision >= 355 and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {352, 353, 354, 355, 351} and not state['owner_approval']:
    ...
    if revision in {352, 353, 354, 355, 351} and not state['session_bound']:
    ...
    if revision in {352, 353, 354, 355, 351} and not state['exit_plan_ready']:
    ...
    if revision in {355} and (not state['permission_ok'] or not state['whitelist_ok']):
    ...
    if revision in {355} and not state['halt_authority']:
    ...
    body = {
        'decision': decision,
        'live_exit_emergency_control': decision,
        'revision': revision,
        'label': label,
    ...
    return {'engine': 'live_exit_emergency_control_contract', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'next_controlled_block'}

def build_rev351_exit_plan_final_contract(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(351, 'exit_plan_final_contract', 'Exit Plan Final Contract', data, settings, auth_store, username)

def build_rev352_sl_tp_trailing_timeout_binding(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(352, 'sl_tp_trailing_timeout_binding', 'Sl Tp Trailing Timeout Binding', data, settings, auth_store, username)

def build_rev353_manual_attention_emergency_close_guard(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(353, 'manual_attention_emergency_close_guard', 'Manual Attention Emergency Close Guard', data, settings, auth_store, username)

def build_rev354_close_blocked_by_default_proof(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(354, 'close_blocked_by_default_proof', 'Close Blocked By Default Proof', data, settings, auth_store, username)

def build_rev355_live_exit_emergency_control_packet(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(355, 'live_exit_emergency_control_packet', 'Live Exit Emergency Control Packet', data, settings, auth_store, username)


_BUILDERS = {
    351: build_rev351_exit_plan_final_contract,
    352: build_rev352_sl_tp_trailing_timeout_binding,
    353: build_rev353_manual_attention_emergency_close_guard,
    354: build_rev354_close_blocked_by_default_proof,
    355: build_rev355_live_exit_emergency_control_packet,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for Live Exit & Emergency Control Contract') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'live_exit_emergency_control': body.get('live_exit_emergency_control') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(351, 355+1)}
    final = parts[355].get(_REV_KEYS[355], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'live_exit_emergency_control_contract',
        'revision': 355,
        ...
        'rev351_exit_plan_final_contract': parts[351],
        'rev352_sl_tp_trailing_timeout_binding': parts[352],
        'rev353_manual_attention_emergency_close_guard': parts[353],
        'rev354_close_blocked_by_default_proof': parts[354],
        'rev355_live_exit_emergency_control_packet': parts[355],
        'live_exit_emergency_control_packet': final,
        'summary_result': build_summary_for_revision(355, data, settings, auth_store, username),
        'checks': checks,
