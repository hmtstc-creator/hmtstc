...
def _make_payload(revision: int, payload_key: str, label: str, data: dict | None, settings: dict | None, auth_store: dict | None, username: str) -> dict:
    ...
    if revision >= 336 and state['sample_count'] < cfg['min_live_sample_count']:
    ...
    if revision >= 336 and state['evidence_confidence'] < cfg['min_evidence_confidence']:
    ...
    if revision in {340} and not state['owner_approval']:
    ...
    if revision in {340} and not state['session_bound']:
    ...
    if revision in {340} and (not state['permission_ok'] or not state['whitelist_ok']):
    ...
    if revision in {340} and not state['halt_authority']:
    ...
    body = {
        'decision': decision,
        'small_cap_autonomy_final_validation': decision,
        'revision': revision,
        'label': label,
    ...
    return {'engine': 'small_cap_autonomy_final_validation', 'revision': revision, 'status': status, 'generated_at': now_iso(), payload_key: body, 'checks': checks, 'check_totals': _totals(checks), 'command_preview': _command_preview(), 'contains_secret': False, 'secret_values_returned': False, 'next_allowed_step': 'next_controlled_block'}

def build_rev336_rev335_autonomy_candidate_evidence_recheck(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(336, 'rev335_autonomy_candidate_evidence_recheck', 'Rev335 Autonomy Candidate Evidence Recheck', data, settings, auth_store, username)

def build_rev337_small_cap_risk_capital_final_validator(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(337, 'small_cap_risk_capital_final_validator', 'Small Cap Risk Capital Final Validator', data, settings, auth_store, username)

def build_rev338_permission_ladder_owner_approval_final_check(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(338, 'permission_ladder_owner_approval_final_check', 'Permission Ladder Owner Approval Final Check', data, settings, auth_store, username)

def build_rev339_summary_v4_decision_consistency_validator(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(339, 'summary_v4_decision_consistency_validator', 'Summary V4 Decision Consistency Validator', data, settings, auth_store, username)

def build_rev340_small_cap_autonomy_final_validation_packet(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    return _make_payload(340, 'small_cap_autonomy_final_validation_packet', 'Small Cap Autonomy Final Validation Packet', data, settings, auth_store, username)


_BUILDERS = {
    336: build_rev336_rev335_autonomy_candidate_evidence_recheck,
    337: build_rev337_small_cap_risk_capital_final_validator,
    338: build_rev338_permission_ladder_owner_approval_final_check,
    339: build_rev339_summary_v4_decision_consistency_validator,
    340: build_rev340_small_cap_autonomy_final_validation_packet,
}
...
def build_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    try:
        ...
        raise ValueError(f'Unsupported Rev{revision} for Small-Cap Autonomy Final Validation') from exc
...
def build_summary_for_revision(revision: int, data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    ...
    return {
        ...
        'small_cap_autonomy_final_validation': body.get('small_cap_autonomy_final_validation') or body.get('decision') or payload.get('status'),
...
def build_block_payload(data: dict | None, settings: dict | None = None, auth_store: dict | None = None, username: str = 'default') -> dict:
    parts = {rev: build_for_revision(rev, data, settings, auth_store, username) for rev in range(336, 340+1)}
    final = parts[340].get(_REV_KEYS[340], {})
    checks = []
    for p in parts.values():
        checks.extend(p.get('checks', []))
    return {
        'engine': 'small_cap_autonomy_final_validation',
        'revision': 340,
        ...
        'rev336_rev335_autonomy_candidate_evidence_recheck': parts[336],
        'rev337_small_cap_risk_capital_final_validator': parts[337],
        'rev338_permission_ladder_owner_approval_final_check': parts[338],
        'rev339_summary_v4_decision_consistency_validator': parts[339],
        'rev340_small_cap_autonomy_final_validation_packet': parts[340],
        'small_cap_autonomy_final_validation_packet': final,
        'summary_result': build_summary_for_revision(340, data, settings, auth_store, username),
        'checks': checks,
